...
def check_line(canvas, stack, cur_pixel, xr, fill_colour, border_colour, dy):
    ...
    while x <= xr:
        ...
        while get_pixel_color(canvas, x, y) != fill_colour and \
                get_pixel_color(canvas, x, y) != border_colour and x <= xr: #Идем пока наш интервал незакрашенных пикселей не прерван
            flag = True
            x += 1
...
def line_by_line_filling_algorithm_with_seed(canvas, border_colour, fill_colour, seed_point, delay=0):
    ...
    while stack:
        ...
        if save_y < canvas.image.height():
            check_line(canvas, stack, [xl, save_y], xr, fill_colour, border_colour, 1) #Проходим по строке снизу y = y +1
        if (save_y > 0):
            check_line(canvas, stack, [xl, save_y], xr, fill_colour, border_colour, -1)#Проходим по строке сверху y = y - 1
